...
# 1. Project Statement
Sarcasm detection in multimodal content presents many challenges requiring sophisticated modeling of cross-modal incongruities and emotional context. This paper introduces HCI-EASD (Hierarchical Cross-Modal Incongruity and Emotion-Aware Sarcasm Detection), a novel architecture that explicitly captures intentional mismatches between textual and visual content characteristic of sarcastic expressions. The model integrates four key modules: multimodal feature extraction using BERT and ResNet-50, emotion-aware processing with incongruity correlation analysis, cross-modal incongruity detection through multi-head attention, and hierarchical classification with compound loss functions. Our approach validates the hypothesis that sarcasm emerges from quantifiable cross-modal discord, enabling detection of both explicit semantic patterns and implicit contradictions. The model is evaluated on MEMOTION, MUStARD and MMSD 2.0 datasets, and demonstrates exceptional performance with accuracy of 0.97, AUC of 0.991, calibration error of 0.0994, and F1-scores exceeding 0.95 for non-sarcastic content while maintaining robust sarcastic classification above 0.6. With MMSD dataset the model achieved 0.95 accuracy. The Analysis of incongruity score, feature attribution, and token-level visualization confirms optimization of the proposed HCI-EASD model for multimodal sarcasm detection.
...
The HCI-EASD architecture as consists of four interconnected modules: <br>(1) Multimodal Feature Extraction,<br> (2) Emotion-Aware Processing,<br> (3) Cross-Modal Incongruity Module<br> (4) Hierarchical Classification. This design enables the model to capture both explicit semantic content and implicit incongruities that characterize sarcastic expressions.
...
## 5. Conclusion
This paper introduced HCI-EASD, a novel hierarchical architecture for multimodal sarcasm detection that successfully addresses the challenge of identifying sarcastic expressions through explicit cross-modal incongruity modeling and emotion-aware processing. The Cross-Modal Incongruity Module effectively captures deliberate mismatches between textual and visual content, while the emotion-aware component enhances detection by analyzing emotional context patterns. Experimental validation on MEMOTION, MUStARD and MMSD 2.0 datasets demonstrates exceptional performance with AUC of 0.991 and calibration error of 0.0994, confirming reliable uncertainty quantification. The model achieves balanced performance with F1-scores exceeding 0.95 for non-sarcastic content and robust sarcastic classification above 0.6. Comprehensive interpretability analysis through feature attribution and token-level visualization reveals meaningful decision-making patterns, enhancing practical deployment trust. Future work should explore extension to other modality combinations, integration with large language models, and cross-cultural sarcasm patterns. The HCI-EASD framework establishes a significant contribution to multimodal learning, providing both superior performance and enhanced interpretability for real-world sarcasm detection applications in social media analysis and content moderation systems.
